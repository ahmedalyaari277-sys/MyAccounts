...
class CustodyViewModel(app: Application): AndroidViewModel(app) {
    ...
    // These flows must be created once per key. Creating stateIn() from a
    // composable-facing getter on every recomposition can replace the flow
    // being collected and cause repeated recomposition/flicker on custody
    // screens, especially when several flows are collected together.
    ...
    fun custody(id: Long): StateFlow<CustodyEntity?> =
        custodyFlows.getOrPut(id) {
            repo.observeCustody(id).stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), null)
        }

    fun persons(id: Long): StateFlow<List<CustodyPersonEntity>> =
        personFlows.getOrPut(id) {
            repo.observePersons(id).stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
        }

    fun accounts(id: Long): StateFlow<List<CustodyAccountEntity>> =
        accountFlows.getOrPut(id) {
            repo.observeAccounts(id).stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
        }

    fun transactions(id: Long): StateFlow<List<CustodyTransactionEntity>> =
        transactionFlows.getOrPut(id) {
    ...
    fun balance(accountId: Long): StateFlow<Long> =
        balanceFlows.getOrPut(accountId) {
