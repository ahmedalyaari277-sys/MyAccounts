...
@Composable
fun PersonAccountScreen(personWithAccounts: PersonWithAccounts, onBack: () -> Unit, onUpdatePerson: (String, String, String, String) -> Unit, onDeletePerson: () -> Unit, onReportClick: (String) -> Unit, transactionViewModel: TransactionViewModel) {
    ...
    Scaffold(containerColor = MaterialTheme.colorScheme.background, topBar = {
        ...
        Column(Modifier.fillMaxSize().padding(paddingValues).padding(horizontal = 16.dp, vertical = 10.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            InformationCard {
            ...
            Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                Text("الحساب والحركات", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
                Row(verticalAlignment = Alignment.CenterVertically) {
                    StatusChip("${accounts.size} عملات")
                    IconButton(onClick = { showReportTypeDialog = true }) { Icon(Icons.Default.MoreVert, contentDescription = "التقارير") }
                }
            }
            if (initialAccount != null) {
                TransactionScreen(accountId = initialAccount.id, currencyCode = initialAccount.currencyCode, accounts = accounts, onBack = {}, transactionViewModel = transactionViewModel, embedded = true, modifier = Modifier.weight(1f), personName = person.name)
            } else {
                InformationCard(modifier = Modifier.weight(1f)) { Text("لا توجد حسابات عملات لهذا الشخص.", style = MaterialTheme.typography.bodyLarge) }
            }
        }
    }
    if (showReportTypeDialog) {
        AlertDialog(onDismissRequest = { showReportTypeDialog = false }, title = { Text("نوع التقرير", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold) }, text = {
    ...
    if (showEditDialog) {
        EditPersonDialog(person.name, person.phone, person.address, person.notes, onDismiss = { showEditDialog = false }) { name, phone, address, notes -> onUpdatePerson(name, phone, address, notes); showEditDialog = false }
    }
    if (showDeleteDialog) {
        AlertDialog(onDismissRequest = { showDeleteDialog = false }, title = { Text("أرشفة الشخص", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold) }, text = { Text("هل أنت متأكد من أرشفة هذا الشخص؟ سيتم إخفاؤه من القائمة الرئيسية مع الاحتفاظ بسجله المالي.", style = MaterialTheme.typography.bodyLarge) }, confirmButton = { DangerButton(text = "أرشفة", onClick = { showDeleteDialog = false; onDeletePerson() }) }, dismissButton = { SecondaryButton(text = "إلغاء", onClick = { showDeleteDialog = false }) })
    }
}
...
private fun EditPersonDialog(name: String, phone: String, address: String, notes: String, onDismiss: () -> Unit, onSave: (String, String, String, String) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text("تعديل بيانات الشخص", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold) }, text = {
        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            androidx.compose.material3.OutlinedTextField(value = editedName, onValueChange = { editedName = it; nameError = false }, modifier = Modifier.fillMaxWidth(), label = { Text("الاسم") }, singleLine = true, isError = nameError, shape = MaterialTheme.shapes.small)
            if (nameError) Text("الاسم مطلوب", color = MaterialTheme.colorScheme.error, style = MaterialTheme.typography.bodySmall)
            androidx.compose.material3.OutlinedTextField(value = editedPhone, onValueChange = { editedPhone = it }, modifier = Modifier.fillMaxWidth(), label = { Text("الهاتف") }, singleLine = true, shape = MaterialTheme.shapes.small)
            androidx.compose.material3.OutlinedTextField(value = editedAddress, onValueChange = { editedAddress = it }, modifier = Modifier.fillMaxWidth(), label = { Text("العنوان") }, shape = MaterialTheme.shapes.small)
            androidx.compose.material3.OutlinedTextField(value = editedNotes, onValueChange = { editedNotes = it }, modifier = Modifier.fillMaxWidth(), label = { Text("الملاحظات") }, minLines = 2, shape = MaterialTheme.shapes.small)
        }
    }, confirmButton = { PrimaryButton(text = "حفظ", onClick = { if (editedName.isBlank()) nameError = true else onSave(editedName.trim(), editedPhone.trim(), editedAddress.trim(), editedNotes.trim()) }) }, dismissButton = { SecondaryButton(text = "إلغاء", onClick = onDismiss) })
}

private fun currencyOrder(code: String): Int = when (code) { "YER" -> 0; "SAR" -> 1; "USD" -> 2; else -> 3 }
private fun balanceStatus(balanceMinor: Long): BalanceStatus = when { balanceMinor > 0L -> BalanceStatus.Due; balanceMinor < 0L -> BalanceStatus.Owed; else -> BalanceStatus.Neutral }
private fun formatBalance(balanceMinor: Long): String { val amount = BigDecimal(balanceMinor).movePointLeft(2).stripTrailingZeros().toPlainString(); return if (balanceMinor > 0L) "+$amount" else amount }
