...
fun CustodyTransferScreen(vm: CustodyViewModel, onBack: () -> Unit) {
    ...
    val exportLauncher = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument(CustodyTwoSheetExcelDataManager.MIME_TYPE)) { uri ->
        if (uri != null) {
            ...
            scope.launch(Dispatchers.IO) {
                ...
                message = r.fold(
                    { "تم تصدير ${it.custodies} عهدة و${it.transactions} عملية إلى Excel. الملف يحتوي Sheetين فقط: بيانات العُهَد وملخص التصنيفات." },
    ...
    pendingImport?.let { uri -> AlertDialog(onDismissRequest = { if (!busy) pendingImport = null }, title = { Text("تأكيد استيراد العُهَد") }, text = { Text("سيتم فحص ملف Excel ذي الورقتين ثم استيراد بيانات العُهَد. لن تتأثر بيانات دفتر الحسابات.") }, confirmButton = { TextButton(enabled = !busy, onClick = { pendingImport = null; busy = true; scope.launch(Dispatchers.IO) { val r = runCatching { val p = CustodyTwoSheetExcelDataManager.previewImport(context, uri).getOrThrow(); check(p.isValid) { p.errors.joinToString("\n") }; CustodyTwoSheetExcelDataManager.import(context, uri).getOrThrow() }; message = r.fold({ "تم الاستيراد: ${it.custodiesAdded} عهدة، ${it.peopleAdded} أشخاص، ${it.accountsAdded} حسابات، ${it.transactionsAdded} عمليات." }, { "تعذر استيراد بيانات العُهَد: ${it.message ?: "ملف غير صالح"}" }); busy = false } }) { Text("استيراد") } }, dismissButton = { TextButton(enabled = !busy, onClick = { pendingImport = null }) { Text("إلغاء") } }) }
    pendingRestore?.let { uri -> AlertDialog(onDismissRequest = { if (!busy) pendingRestore = null }, title = { Text("تأكيد استعادة العُهَد") }, text = { Text("سيتم استيراد بيانات العُهَد من النسخة الاحتياطية، مع إعادة المرفقات إن وجدت. لن يتم تعديل بيانات دفتر الحسابات.") }, confirmButton = { TextButton(enabled = !busy, onClick = { pendingRestore = null; busy = true; scope.launch(Dispatchers.IO) { val r = CustodyBackupManager.restoreBackup(context, uri); message = r.fold({ "تمت استعادة العُهَد: ${it.custodies} عهدة، ${it.people} أشخاص، ${it.transactions} عمليات، ${it.attachments} مرفقات." }, { "تعذرت استعادة نسخة العُهَد: ${it.message ?: "الملف غير صالح"}" }); busy = false } }) { Text("استعادة") } }, dismissButton = { TextButton(enabled = !busy, onClick = { pendingRestore = null }) { Text("إلغاء") } }) }
