...
fun CustodyOwnerEditDialog(vm: CustodyViewModel, custody: CustodyEntity, onDismiss: () -> Unit, onSaved: () -> Unit) {
    var name by remember { mutableStateOf(custody.name) }; var phone by remember { mutableStateOf(custody.phone) }; var address by remember { mutableStateOf(custody.address) }; var notes by remember { mutableStateOf(custody.notes) }; var saving by remember { mutableStateOf(false) }; var error by remember { mutableStateOf<String?>(null) }; val scope = rememberCoroutineScope()
    AlertDialog(onDismissRequest = { if (!saving) onDismiss() }, title = { Text("تعديل بيانات حامل العهدة") }, text = { Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).imePadding(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        OutlinedTextField(name, { name = it }, Modifier.fillMaxWidth().keepFocusedFieldVisible(), label = { Text("اسم حامل العهدة") }, singleLine = true, enabled = !saving); OutlinedTextField(phone, { phone = it }, Modifier.fillMaxWidth().keepFocusedFieldVisible(), label = { Text("الهاتف") }, singleLine = true, enabled = !saving); OutlinedTextField(address, { address = it }, Modifier.fillMaxWidth().keepFocusedFieldVisible(), label = { Text("العنوان") }, singleLine = true, enabled = !saving); OutlinedTextField(notes, { notes = it }, Modifier.fillMaxWidth().keepFocusedFieldVisible(), label = { Text("الملاحظات") }, minLines = 2, enabled = !saving); error?.let { Text(it, color = MaterialTheme.colorScheme.error) }
    } }, confirmButton = { Button(enabled = name.isNotBlank() && !saving, onClick = { saving = true; scope.launch { runCatching { vm.updateCustodyAndWait(custody.copy(name = name.trim(), phone = phone.trim(), address = address.trim(), notes = notes.trim())) }.onSuccess { saving = false; onSaved() }.onFailure { saving = false; error = it.message ?: "تعذر حفظ التعديل" } } }) { Text(if (saving) "جارٍ الحفظ…" else "حفظ") } }, dismissButton = { TextButton(enabled = !saving, onClick = onDismiss) { Text("إلغاء") } })
}

@Composable
fun CustodyDataEditDialog(vm: CustodyViewModel, custody: CustodyEntity, onDismiss: () -> Unit, onSaved: () -> Unit) {
    var organization by remember { mutableStateOf(custody.organizationName) }; var organizationPhone by remember { mutableStateOf(custody.organizationPhone) }; var organizationAddress by remember { mutableStateOf(custody.organizationAddress) }; var organizationNotes by remember { mutableStateOf(custody.organizationNotes) }; var saving by remember { mutableStateOf(false) }; var error by remember { mutableStateOf<String?>(null) }; val scope = rememberCoroutineScope()
    AlertDialog(onDismissRequest = { if (!saving) onDismiss() }, title = { Text("تعديل بيانات العهدة والجهة") }, text = { Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).imePadding(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
