...
fun ArchivedPersonDetailScreen(
    ...
    var showRestoreMergeDialog by remember { mutableStateOf(false) }
    val person = personWithAccounts.person

    fun requestRestore() {
        if (hasActiveSameName) showRestoreMergeDialog = true else onRestore()
    }
    ...
    Scaffold(
        ...
        LazyColumn(modifier = Modifier.fillMaxSize().padding(paddingValues).padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            item {
            ...
            if (archivedTransactions.isEmpty()) {
                ...
                items(archivedTransactions, key = { it.id }) { transaction ->
                    ArchivedPersonTransactionCard(transaction = transaction, onRestore = { onRestoreTransaction(transaction.id) })
                }
            }
            item {
                Spacer(Modifier.height(6.dp))
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(onClick = ::requestRestore, modifier = Modifier.weight(1f)) {
    ...
    if (showRestoreMergeDialog) {
        AlertDialog(
            onDismissRequest = { showRestoreMergeDialog = false },
            title = { Text("تأكيد استعادة الحساب") },
            text = {
                Text(
                    "يوجد بالفعل حساب نشط باسم «${person.name}». إذا تابعت، سيتم استخدام الحساب الموجود بدل إنشاء حساب جديد، وستُنقل إليه العمليات المحفوظة ضمن الحساب المؤرشف. هل تريد المتابعة؟"
                )
            },
            confirmButton = {
                Button(onClick = {
                    showRestoreMergeDialog = false
                    onRestore()
                }) { Text("متابعة الاستعادة") }
            },
            dismissButton = { TextButton(onClick = { showRestoreMergeDialog = false }) { Text("إلغاء") } }
        )
    }
    ...
    if (showDeleteDialog) {
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = { showDeleteDialog = false; onPermanentDelete() }) { Text("حذف نهائي") }
            },
...
@Composable
private fun ArchivedPersonTransactionCard(transaction: TransactionEntity, onRestore: () -> Unit) {
    ...
    androidx.compose.material3.Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                ...
                IconButton(onClick = onRestore) { Icon(Icons.Default.Restore, contentDescription = "استعادة العملية", tint = MaterialTheme.colorScheme.primary) }
            }
        }
    }
}
