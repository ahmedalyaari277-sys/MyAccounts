...
fun AppNavHost(navController: NavHostController, viewModel: LedgerViewModel, appearanceMode: AppearanceMode, onAppearanceModeChange: (AppearanceMode) -> Unit) {
    ...
    NavHost(navController = navController, startDestination = Routes.GATEWAY) {
        ...
        composable(Routes.SETTINGS) { SettingsScreen(security, appearanceMode, onAppearanceModeChange, { navController.popBackStack() }, { navController.navigate(Routes.DETAILS) }, { navController.navigate(Routes.BACKUP_RESTORE) }) }
        composable(Routes.DETAILS) { DetailsScreen { navController.popBackStack() } }
        composable(Routes.CUSTODIES) { CustodyHomeWithArchiveScreen(custody, { navController.popBackStack() }, { navController.navigate(Routes.custody(it)) }, { navController.navigate(Routes.CUSTODY_ARCHIVE) }, { navController.navigate(Routes.CUSTODY_REPORTS) }, { navController.navigate(Routes.BACKUP_RESTORE) }, { navController.navigate(Routes.CUSTODY_TRANSFER) }) }
        ...
        composable(Routes.CUSTODY, arguments = listOf(navArgument("custodyId") { type = NavType.LongType })) { e -> e.arguments?.getLong("custodyId")?.let { id -> CustodyOverviewScreen(custody, id, { navController.popBackStack() }, { navController.navigate(Routes.custodyHolder(id)) }, { personId -> navController.navigate(Routes.custodyPerson(id, personId)) }) } }
        composable(Routes.CUSTODY_HOLDER, arguments = listOf(navArgument("custodyId") { type = NavType.LongType })) { e -> e.arguments?.getLong("custodyId")?.let { id -> CustodyOperationsScreen(custody, id, { navController.popBackStack() }, { personId -> navController.navigate(Routes.custodyPerson(id, personId)) }) } }
