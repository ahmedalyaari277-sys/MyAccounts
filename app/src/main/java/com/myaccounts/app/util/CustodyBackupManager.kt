...
object CustodyBackupManager {
    ...
    private fun readZip(context: Context, uri: Uri): Map<String, ByteArray> {
        val input = context.contentResolver.openInputStream(uri) ?: error("تعذر فتح ملف نسخة العهد الاحتياطية.")
        val result = mutableMapOf<String, ByteArray>()
        ZipInputStream(input.buffered()).use { zip -> while (true) { val entry = zip.nextEntry ?: break; if (entry.isDirectory) continue; val out = ByteArrayOutputStream(); zip.copyTo(out); result[entry.name] = out.toByteArray() } }
        return result
    }
}
