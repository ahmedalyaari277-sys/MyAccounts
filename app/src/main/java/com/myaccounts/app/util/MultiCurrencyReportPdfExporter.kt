...
object MultiCurrencyReportPdfExporter {
    ...
    fun exportPeopleReport(context: Context, summaries: List<CurrencyReportSummary>, people: List<CurrencyReportPersonRowWithCurrency>, start: Long?, end: Long?): Result<String> = save(context, "تقرير الأشخاص", buildPeople(summaries, people, start, end))
    fun exportDetailedReport(context: Context, summaries: List<CurrencyReportSummary>, transactions: List<GeneralReportTransactionRow>, start: Long?, end: Long?): Result<String> = save(context, "التقرير العام", buildDetailed(summaries, transactions, start, end))
    fun exportSummaryReport(context: Context, rows: List<PersonCurrencySummaryRow>, start: Long?, end: Long?): Result<String> = save(context, "أرصدة الحسابات", buildSummary(rows, start, end))
    fun exportPersonReport(context: Context, report: MultiCurrencyPersonReport, start: Long?, end: Long?): Result<String> = save(context, "تقرير حساب ${report.personName}", buildPerson(report, start, end))
    ...
    private fun buildPeople(summaries: List<CurrencyReportSummary>, people: List<CurrencyReportPersonRowWithCurrency>, start: Long?, end: Long?): PdfDocument {
        ...
        pages.forEachIndexed { i, chunk ->
            ...
            val cols = peopleColumns(); tableHeader(c, 105f, cols, 1, 4); var y = 143f
            chunk.forEach { (id, name) ->
                val cells = mutableListOf(name)
                currencies.forEach { cur -> val p = people.firstOrNull { it.personId == id && it.currencyCode == cur }; cells += listOf(format(p?.totalReceivableMinor ?: 0), format(p?.totalPayableMinor ?: 0), balanceText(p?.balanceMinor ?: 0), "${p?.transactionCount ?: 0}") }
                row(c, y, cols, cells); y += 32f
            }
            if (i == pages.lastIndex) totalPeople(c, y, cols, people); footer(c, i + 1, pages.size); doc.finishPage(page)
        }
        return doc
    }
    ...
    private fun buildDetailed(summaries: List<CurrencyReportSummary>, transactions: List<GeneralReportTransactionRow>, start: Long?, end: Long?): PdfDocument {
        val doc = PdfDocument(); val chunks = transactions.chunked(10).ifEmpty { listOf(emptyList()) }
        chunks.forEachIndexed { i, chunk ->
            ...
            if (i == chunks.lastIndex) totalDetailed(c, y, cols, summaries); footer(c, i + 1, chunks.size); doc.finishPage(page)
        }
        return doc
    }

    private fun buildSummary(rowsData: List<PersonCurrencySummaryRow>, start: Long?, end: Long?): PdfDocument {
        val doc = PdfDocument(); val persons = rowsData.map { it.personId to it.personName }.distinctBy { it.first }.sortedBy { it.second.lowercase() }; val pages = persons.chunked(10).ifEmpty { listOf(emptyList()) }
        pages.forEachIndexed { i, chunk ->
            val page = doc.startPage(PdfDocument.PageInfo.Builder(W, H, i + 1).create()); val c = page.canvas
            header(c, "تقرير أرصدة الحسابات — جميع العملات", "الفترة: ${range(start, end)}")
            val cols = summaryColumns(); tableHeader(c, 105f, cols, 1, 4); var y = 143f
            chunk.forEach { (id, name) ->
                val cells = mutableListOf(name)
                currencies.forEach { cur -> val r = rowsData.firstOrNull { it.personId == id && it.currencyCode == cur }; cells += listOf(format(r?.totalReceivableMinor ?: 0), format(r?.totalPayableMinor ?: 0), balanceText(r?.balanceMinor ?: 0), "${r?.transactionCount ?: 0}") }
                row(c, y, cols, cells); y += 32f
            }
            if (i == pages.lastIndex) totalSummary(c, y, cols, rowsData); footer(c, i + 1, pages.size); doc.finishPage(page)
        }
        return doc
    }

    private fun buildPerson(report: MultiCurrencyPersonReport, start: Long?, end: Long?): PdfDocument {
        val doc = PdfDocument(); val transactions = report.reports.flatMap { r -> r.transactions.map { r.currencyCode to it } }.sortedWith(compareBy<Pair<String, com.myaccounts.app.data.reports.PersonReportTransaction>> { it.second.transactionDate }.thenBy { it.second.transactionId }); val pages = transactions.chunked(10).ifEmpty { listOf(emptyList()) }
        pages.forEachIndexed { i, chunk ->
            val page = doc.startPage(PdfDocument.PageInfo.Builder(W, H, i + 1).create()); val c = page.canvas
            ...
            val cols = personColumns(); tableHeader(c, 105f, cols, 2, 3); var y = 143f
            chunk.forEach { (cur, t) ->
                val cells = mutableListOf(date(t.transactionDate), t.description.ifBlank { "—" })
                currencies.forEach { cc -> if (cc == cur) cells += listOf(if (t.type == "RECEIVABLE") format(t.amountMinor) else "—", if (t.type == "PAYABLE") format(t.amountMinor) else "—", balanceText(t.balanceMinor)) else cells += listOf("—", "—", "—") }
                row(c, y, cols, cells); y += 32f
            }
            if (i == pages.lastIndex) totalPerson(c, y, cols, report); footer(c, i + 1, pages.size); doc.finishPage(page)
        }
        return doc
    }
    ...
    private fun header(c: Canvas, title: String, info: String) { c.drawText(title, RIGHT, 34f, paint(18f, true, Color.rgb(30, 30, 30))); c.drawText(info, RIGHT, 58f, paint(9f, false, Color.DKGRAY)); c.drawLine(LEFT, 70f, RIGHT, 70f, line()) }

    private fun tableHeader(c: Canvas, y: Float, cols: List<Col>, baseCount: Int, groupSize: Int) {
        val bg = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = BLUE }; val white = paint(8f, true, Color.WHITE); val groupY = y - 28f
        cols.take(baseCount).forEach { col -> c.drawRect(col.left, groupY, col.right, y + 18f, bg); c.drawText(col.title, (col.left + col.right) / 2f, y - 3f, centerPaint(white)) }
        var index = baseCount
        currencies.forEach { cur ->
            val first = cols[index]; val last = cols[index + groupSize - 1]; c.drawRect(first.left, groupY, last.right, y - 9f, bg); c.drawText(currencyName(cur), (first.left + last.right) / 2f, groupY + 14f, centerPaint(white))
            for (j in 0 until groupSize) { val col = cols[index + j]; c.drawRect(col.left, y - 9f, col.right, y + 18f, bg); c.drawText(col.title, (col.left + col.right) / 2f, y + 9f, centerPaint(white)) }
            index += groupSize
        }
        cols.forEach { col -> c.drawLine(col.left, groupY, col.left, y + 18f, line()) }; c.drawLine(LEFT, y + 18f, RIGHT, y + 18f, line())
    }

    private fun row(c: Canvas, y: Float, cols: List<Col>, values: List<String>) {
        cols.forEachIndexed { i, col ->
            c.drawLine(col.left, y - 18f, col.left, y + 18f, line()); val v = values.getOrNull(i).orEmpty(); val color = if (v.contains("عليه")) RED else if (v.contains("له")) GREEN else Color.rgb(35, 35, 35)
    ...
    private fun totalPeople(c: Canvas, y: Float, cols: List<Col>, data: List<CurrencyReportPersonRowWithCurrency>) {
        val values = mutableListOf("المجموع"); currencies.forEach { cur -> val x = data.filter { it.currencyCode == cur }; val a = x.sumOf { it.totalReceivableMinor }; val l = x.sumOf { it.totalPayableMinor }; values += listOf(format(a), format(l), balanceText(a - l), x.sumOf { it.transactionCount }.toString()) }; row(c, y, cols, values)
    }

    private fun totalDetailed(c: Canvas, y: Float, cols: List<Col>, summaries: List<CurrencyReportSummary>) {
        val values = mutableListOf("المجموع", "", ""); currencies.forEach { cur -> val x = summaries.firstOrNull { it.currencyCode == cur }; values += listOf(format(x?.totalReceivableMinor ?: 0), format(x?.totalPayableMinor ?: 0), balanceText(x?.balanceMinor ?: 0)) }; row(c, y, cols, values)
    }
    ...
    private fun totalPerson(c: Canvas, y: Float, cols: List<Col>, report: MultiCurrencyPersonReport) {
        val values = mutableListOf("المجموع", ""); currencies.forEach { cur -> val s = report.reports.firstOrNull { it.currencyCode == cur }?.summary; val a = s?.periodReceivableMinor ?: 0; val l = s?.periodPayableMinor ?: 0; values += listOf(format(a), format(l), balanceText(a - l)) }; row(c, y, cols, values)
    }
    ...
    private fun save(context: Context, title: String, document: PdfDocument): Result<String> = try {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            val resolver = context.contentResolver; val uri = resolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI, values) ?: throw IllegalStateException("تعذر إنشاء ملف PDF.")
            try { resolver.openOutputStream(uri).use { out -> if (out == null) throw IllegalStateException("تعذر فتح ملف PDF."); document.writeTo(out) }; resolver.update(uri, ContentValues().apply { put(MediaStore.Downloads.IS_PENDING, 0) }, null, null) } catch (e: Exception) { resolver.delete(uri, null, null); throw e }
            Result.success("تم حفظ تقرير PDF في مجلد التنزيلات/MyAccounts")
        } else {
            val dir = File(context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS), "MyAccounts"); if (!dir.exists() && !dir.mkdirs()) throw IllegalStateException("تعذر إنشاء مجلد التقرير."); File(dir, fileName).also { FileOutputStream(it).use { out -> document.writeTo(out) } }; Result.success("تم حفظ تقرير PDF في مجلد MyAccounts")
        }
