...
class CustodyRepository(private val db: com.myaccounts.app.data.local.AppDatabase, context: Context) {
    ...
    private val currencies: List<String> get() = CurrencyCatalog.definitions.map { it.code }
    ...
    suspend fun addTransaction(custodyId: Long, currency: String, type: String, personId: Long?, amountMinor: Long, description: String, date: Long, attachments: List<CustodyAttachmentStorage.Selected> = emptyList()): Long = withContext(Dispatchers.IO) {
        ...
        val ownerDelta = CustodyBalanceRules.ownerDelta(type, amountMinor); val personDelta = CustodyBalanceRules.personDelta(type, amountMinor)
        ...
        try {
            ...
            if (transactionId != 0L) {
                ...
                runCatching { db.withTransaction { dao.getTransaction(transactionId)?.let { old -> dao.adjustAccountBalance(old.accountId, -CustodyBalanceRules.ownerDelta(old.type, old.amountMinor), System.currentTimeMillis()); if (old.personId != null) dao.getPersonAccount(old.custodyId, old.personId, old.currencyCode)?.let { pa -> val pd = CustodyBalanceRules.personDelta(old.type, old.amountMinor); if (pd != 0L) dao.adjustAccountBalance(pa.id, -pd, System.currentTimeMillis()) }; dao.deleteTransaction(transactionId) } } }
            }
            throw e
        }
    }
    ...
    suspend fun updateTransaction(id: Long, currency: String, type: String, personId: Long?, amountMinor: Long, description: String, date: Long, newAttachments: List<CustodyAttachmentStorage.Selected> = emptyList(), deletedAttachments: List<CustodyTransactionAttachmentEntity> = emptyList()) = withContext(Dispatchers.IO) {
        ...
        db.withTransaction {
            ...
            if (old.personId != null) dao.getPersonAccount(old.custodyId, old.personId, old.currencyCode)?.let { pa -> val delta = CustodyBalanceRules.personDelta(old.type, old.amountMinor); if (delta != 0L) dao.adjustAccountBalance(pa.id, -delta, now) }
            ...
            if (newPersonAccount != null) { val delta = CustodyBalanceRules.personDelta(type, amountMinor); if (delta != 0L) dao.adjustAccountBalance(newPersonAccount.id, delta, now) }
        }
    ...
    suspend fun deleteTransaction(id: Long) = db.withTransaction {
        ...
        if (old.personId != null) dao.getPersonAccount(old.custodyId, old.personId, old.currencyCode)?.let { pa -> val delta = CustodyBalanceRules.personDelta(old.type, old.amountMinor); if (delta != 0L) dao.adjustAccountBalance(pa.id, -delta, System.currentTimeMillis()) }
