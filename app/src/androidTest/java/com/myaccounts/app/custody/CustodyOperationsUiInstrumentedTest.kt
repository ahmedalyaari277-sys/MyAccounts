...
class CustodyOperationsUiInstrumentedTest {
    ...
    fun custodyGatewayDetailAndOrganizationOperationWorkEndToEnd() = runBlocking {
        openCustodyDetail()
        clickAction("استلام من الجهة")
        waitForOperationDialog(false)
        setField("المبلغ للعملية", "1000")
        clickSaveOperation()
        assertTrue(device.wait(Until.hasObject(By.text("عليه 1000")), 10_000))

        clickAction("مرتجع للجهة / تصفية")
        waitForOperationDialog(false)
        setField("المبلغ للعملية", "1000")
        clickSaveOperation()
        assertTrue(device.wait(Until.hasObject(By.text("متوازن 0")), 10_000))
    ...
    fun custodyPersonAndPersonOperationWorkEndToEndIncludingEditDeleteAndCurrencyChange() = runBlocking {
        ...
        clickClickable(By.desc("حفظ الشخص"), "Save person")
        ...
        clickClickable(By.text("اختبار شخص واجهة"), "Custody person")
        assertTrue(device.wait(Until.hasObject(By.text("صرف للشخص")), 10_000))

        clickAction("إضافة عملية")
        waitForOperationDialog(false)
        setField("المبلغ للعملية", "250")
        setField("بيان العملية", "صرف واجهة")
        clickSaveOperation()
        assertTrue(device.wait(Until.hasObject(By.text("عليه 250")), 10_000))
        ...
        clickClickable(By.text("مرتجع من الشخص"), "Change operation type")
        clickSaveOperation()
        ...
        clickAction("تعديل")
        waitForOperationDialog(true)
        clickClickable(By.text("مرتجع للجهة / تصفية"), "Change back to organization return")
        clickSaveOperation()

        tx = db.custodyDao().getAllTransactions(custody.id, false)
        assertEquals(CustodyTransactionType.RETURNED_TO_ORG, tx.single().type)
    ...
    private fun setField(description: String, value: String) {
        ...
        val field = if (focused?.className == "android.widget.EditText") focused else device.findObjects(By.clazz("android.widget.EditText")).lastOrNull()
            ?: error("Editable field '$description' not found")
        field.text = value
        device.waitForIdle()
    }
