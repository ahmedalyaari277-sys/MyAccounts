...
class CustodyOperationsUiInstrumentedTest {
    ...
    fun setUp() = runBlocking {
        ...
        clearData()
        CustodyRepository(db, context).createCustody(CustodyEntity(name = custodyName, organizationName = "اختبار الجهة $id", externalId = externalId))
    ...
    fun custodyGatewayDetailAndOrganizationOperationWorkEndToEnd() = runBlocking {
        ...
        waitForDetailScreen()
        click(By.desc("استلام من الجهة"), "Receive from organization")
        ...
        click(By.desc("مرتجع للجهة / تصفية"), "Return to organization")
        waitForOperationDialog()
        setField("المبلغ للعملية", "1000")
        clickSaveOperation()
    ...
    fun custodyPersonAndPersonOperationWorkEndToEndIncludingEditDeleteAndCurrencyChange() = runBlocking {
        ...
        click(By.text(custodyName), "Custody card")
        waitForDetailScreen()
        click(By.desc("إضافة شخص"), "Add custody person")
        waitForPersonDialog()
        ...
        click(By.desc("إضافة عملية"), "Add person operation")
        waitForOperationDialog()
    ...
    private fun waitForDetailScreen() = assertTrue("Custody detail screen not visible", device.wait(Until.hasObject(By.desc("شاشة تفاصيل العهدة")), 10_000))
    private fun waitForPersonDialog() {
        assertTrue("Person dialog not visible", device.wait(Until.hasObject(By.desc("الاسم")), 10_000))
        assertTrue("Person dialog fields not visible", device.wait(Until.hasObject(By.desc("الهاتف")), 5_000))
        assertTrue("Person dialog save control not visible", device.wait(Until.hasObject(By.desc("حفظ الشخص")), 5_000))
    }
    private fun clickSavePerson() {
        device.wait(Until.findObject(By.desc("حفظ الشخص")), 5_000)?.click() ?: error("Save person not found")
        device.waitForIdle()
        assertTrue("Person dialog did not close", device.wait(Until.gone(By.desc("حفظ الشخص")), 10_000))
    }
    ...
    private fun clickSaveOperation() {
        device.wait(Until.findObject(By.desc("حفظ العملية")), 5_000)?.click() ?: error("Save operation not found")
        device.waitForIdle()
        assertTrue("Operation dialog did not close", device.wait(Until.gone(By.desc("حفظ العملية")), 10_000))
    }
    ...
    private fun waitForCustody(): CustodyEntity {
        ...
        while (System.currentTimeMillis() < deadline) { runBlocking { db.custodyDao().getCustodyByExternalId(externalId) }?.let { return it }; Thread.sleep(100) }
    ...
    private fun waitForPerson(custodyId: Long, name: String): com.myaccounts.app.data.custody.CustodyPersonEntity {
        val deadline = System.currentTimeMillis() + 10_000L
        while (System.currentTimeMillis() < deadline) { runBlocking { db.custodyDao().getAllPersons(custodyId).firstOrNull { it.name == name } }?.let { return it }; Thread.sleep(100) }
        return runBlocking { db.custodyDao().getAllPersons(custodyId).firstOrNull { it.name == name } }.also { assertNotNull("Test person was not persisted", it) }!!
    }
    private fun waitForTransactions(custodyId: Long, minimum: Int): List<com.myaccounts.app.data.custody.CustodyTransactionEntity> {
        val deadline = System.currentTimeMillis() + 10_000L
        while (System.currentTimeMillis() < deadline) { val transactions = runBlocking { db.custodyDao().getAllTransactions(custodyId, false) }; if (transactions.size >= minimum || (minimum == 0 && transactions.isEmpty())) return transactions; Thread.sleep(100) }
